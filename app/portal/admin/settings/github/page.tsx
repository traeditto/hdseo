...
export default async function GitHubSettingsPage({searchParams}:{searchParams:Promise<{agencyId?:string;github?:string}>}){
  ...
  const user=await requirePortalUser("admin",`/portal/admin/settings/github${params.agencyId?`?agencyId=${encodeURIComponent(params.agencyId)}`:""}`);await upsertLiveUser(user);
