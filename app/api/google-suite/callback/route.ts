...
export async function GET(request:Request){
  ...
  try{
    ...
    const email=await googleAccountEmail(credentials.accessToken),now=new Date().toISOString();let selected:string|null=null,metadata:Record<string,unknown>={connectedAt:now,health:"selection_required"};
    ...
    await auditEvent({agencyId:state.agencyId,actorUserId:state.userId,action:`${purpose}.connected`,resourceType:"integration_connection",resourceId:saved.data.id,afterState:{projectId:state.projectId,selected,email}});
    const destination=new URL(state.returnUrl??"/portal/agency?tab=Results",`${appBaseUrl()}/`);if(destination.origin!==appBaseUrl())destination.href=`${appBaseUrl()}/portal/agency?tab=Results`;destination.searchParams.set(purpose==="google_analytics"?"ga4":"gbp","connected");return new Response(null,{status:303,headers:{location:destination.toString()}});
