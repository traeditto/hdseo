...
import { addVercelProjectDomain, createVercelProject, exchangeVercelCode, getVercelProject, listVercelProjectDomains, listVercelProjects, vercelRequest, type VercelCredentials } from "@/lib/vercel/client";
...
const managedConnectionSchema=z.object({agencyId:z.string().uuid(),clientId:z.string().uuid(),projectId:z.string().uuid(),useManagedPlatform:z.literal(true)});
...
export async function POST(request:Request){try{
  const input=await parseJson(request,z.union([platformConnectionSchema,managedConnectionSchema,connectSchema]));
  ...
  if("useManagedPlatform" in input){
    const context=await requireLiveAgencyProject({projectId:input.projectId,permission:"integrations.manage"});
    if(context.agencyId!==input.agencyId||context.clientId!==input.clientId)throw new ApiError("Managed Vercel setup does not belong to this business.",403,"TENANT_DENIED");
    const subscription=await context.db.from("client_subscriptions").select("plan_key,status").eq("project_id",input.projectId).eq("client_organization_id",input.clientId).maybeSingle();
    if(!subscription.data||!['active','trialing'].includes(subscription.data.status)||!['pro','autopilot_plus'].includes(subscription.data.plan_key))throw new ApiError("Managed Vercel setup is included with Run It for Me and Autopilot plans.",402,"SUBSCRIPTION_REQUIRED");
    if(!env.VERCEL_ACCESS_TOKEN)throw new ApiError("HD SEO managed Vercel access is not configured.",503,"NOT_CONFIGURED");
    const saved=await storeConnection({agencyId:context.agencyId,userId:context.userId,token:env.VERCEL_ACCESS_TOKEN,teamId:env.VERCEL_TEAM_ID??null});
    await auditEvent({agencyId:context.agencyId,actorUserId:context.userId,action:"vercel.connection.managed_platform_bound",resourceType:"vercel_connection",resourceId:saved.id,request,afterState:{projectId:input.projectId,planKey:subscription.data.plan_key}});
    return Response.json({ok:true,connectionId:saved.id,status:"active"});
  }
  const context=await requireLiveAgencyProject({projectId:input.projectId,permission:"integrations.manage"});if(context.agencyId!==input.agencyId||context.clientId!==input.clientId)throw new ApiError("Vercel setup does not belong to this business.",403,"TENANT_DENIED");
  const db=requireAdminDb(),repositoryResult=await db.from("repositories").select("id,full_name,default_branch").eq("id",input.repositoryId).eq("agency_id",context.agencyId).eq("project_id",input.projectId).single();
  ...
  if(input.accessToken){const saved=await storeConnection({agencyId:context.agencyId,userId:context.userId,token:input.accessToken,teamId:input.teamId,teamSlug:input.teamSlug});connectionId=saved.id;credentials={token:input.accessToken,teamId:input.teamId,teamSlug:input.teamSlug};}
  else credentials=await loadVercelCredentials(connectionId!,context.agencyId);
  let providerProject;
  if(input.vercelProjectId)providerProject=await getVercelProject(credentials,input.vercelProjectId);
    ...
    const listed=await listVercelProjects(credentials),matching=listed.projects.find(project=>project.link?.type==="github"&&project.link.org&&project.link.repo&&`${project.link.org}/${project.link.repo}`.toLowerCase()===repositoryResult.data.full_name.toLowerCase());
    if(matching)providerProject=matching;
    else try{providerProject=await getVercelProject(credentials,input.projectName);}
    catch(error){
  ...
  const clientId=await enterpriseClientId(input.clientId,context.agencyId),savedProject=await db.from("vercel_projects").upsert({agency_id:context.agencyId,client_id:clientId,project_id:input.projectId,connection_id:connectionId!,repository_id:input.repositoryId,vercel_project_id:providerProject.id,name:providerProject.name,framework:providerProject.framework??input.framework??null,root_directory:input.rootDirectory??null,production_branch:providerProject.link?.productionBranch??repositoryResult.data.default_branch,production_domains:input.productionDomains,environment_config:{domainVerification:domainResults},status:"active",last_synced_at:new Date().toISOString(),updated_at:new Date().toISOString()},{onConflict:"connection_id,vercel_project_id"}).select("id").single();
  if(!savedProject.data)throw new ApiError("Vercel project connection could not be stored.",500,"OPERATION_FAILED");
  await auditEvent({agencyId:context.agencyId,actorUserId:context.userId,action:"vercel.project.connected",resourceType:"vercel_project",resourceId:savedProject.data.id,request,afterState:{vercelProjectId:providerProject.id,repositoryId:input.repositoryId}});
