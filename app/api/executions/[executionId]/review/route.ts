...
export async function POST(request:Request,{params}:{params:Promise<{executionId:string}>}){try{
  ...
  if(context.agencyId!==input.agencyId||context.clientId!==input.clientId)throw new ApiError("The selected execution does not belong to this workspace.",403,"TENANT_DENIED");
  const{executionId}=await params,db=context.db??createSupabaseAdminClient();if(!db)throw new ApiError("Supabase is not configured.",503,"NOT_CONFIGURED");
  const execution=await db.from("seo_executions").select("id,status,opportunity_id,repository_connection_id,base_branch,base_commit_sha,validation_results").eq("id",executionId).eq("agency_id",context.agencyId).eq("client_organization_id",context.clientId).eq("project_id",context.project.id).maybeSingle();
  ...
  if(input.action==="approve"){
    ...
    const opportunity=await db.from("seo_opportunities").select("opportunity_score,confidence_score,target_milestone,evidence").eq("id",execution.data.opportunity_id).eq("agency_id",context.agencyId).eq("project_id",context.project.id).maybeSingle();if(!opportunity.data)throw new ApiError("The approved opportunity is unavailable.",409,"CONFLICT");
    const plan=repositoryPullRequestPlan({agencyId:context.agencyId,clientId:context.clientId,projectId:context.project.id,executionId,repositoryConnectionId:execution.data.repository_connection_id,baseBranch:execution.data.base_branch,baseCommitSha:execution.data.base_commit_sha,files:(files.data as ExecutionFile[]).map(file=>({path:file.file_path,content:approvedContent(file)})),opportunity:{opportunityScore:opportunity.data.opportunity_score,confidenceScore:opportunity.data.confidence_score,targetMilestone:opportunity.data.target_milestone,evidence:(opportunity.data.evidence??{}) as Record<string,unknown>}}),digest=mutationDigest(plan.action),intent=await requestMutationIntent(db,{action:plan.action,summary:`Create an exact draft pull request with ${plan.files.length} human-reviewed file${plan.files.length===1?"":"s"}.`,riskLevel:"high",approvalPolicy:"human",requestedBy:null,idempotencyKey:`mutation:github-pr:${executionId}:${digest}`,expiresInMinutes:120});
    const approvedIntent=intent.status==="awaiting"?await decideMutationIntent(db,{intentId:intent.id,agencyId:context.agencyId,projectId:context.project.id,actorId:context.userId,decision:"approved"}):intent;
    ...
    const saved=await db.from("seo_executions").update({status:"approved",approved_at:new Date().toISOString(),repository_mutation_intent_id:intent.id,repository_action_digest:digest,validation_results:{...validation,repositoryApproval:{mutationIntentId:intent.id,actionDigest:digest,approvedBy:context.userId,approvedAt:new Date().toISOString()}}}).eq("id",executionId).select("id").maybeSingle();if(saved.error||!saved.data)throw new ApiError("The exact repository approval could not be bound to this execution.",500,"DATABASE_BINDING_FAILED");
    const queued=await db.from("seo_campaign_jobs").update({status:"queued",current_stage:"validate_changes",next_attempt_at:new Date().toISOString()}).contains("result",{executionId}).select("id");if(queued.error||!queued.data?.length)throw new ApiError("The approved execution could not be resumed.",500,"DATABASE_BINDING_FAILED");
