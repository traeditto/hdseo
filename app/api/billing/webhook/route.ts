  ...
  try {
    ...
    if(event.type==="checkout.session.completed"&&kind==="agency_subscription"){
      ...
      if(kind==="agent_capacity"){
        ...
        await db.from("client_subscriptions").update({ plan_key: planKey, price_cents: priceCents, stripe_customer_id: object.customer, stripe_subscription_id: object.subscription, status: "active", trial_ends_at: null,offer_key:beta?FOUNDING_BETA_OFFER_KEY:null,offer_price_cents:beta?plan.beta.priceCents:null,offer_started_at:beta?new Date().toISOString():null,offer_ends_at:offerEndsAt,beta_redeemed_at:beta?new Date().toISOString():undefined, updated_at: new Date().toISOString() }).eq("project_id", projectId);
        if(planKey){const project=await db.from("seo_projects").select("agency_id,client_organization_id").eq("id",projectId).maybeSingle(),client=project.data?await db.from("clients").select("id").eq("agency_id",project.data.agency_id).eq("organization_id",project.data.client_organization_id).maybeSingle():{data:null},subscription=await db.from("client_subscriptions").select("id").eq("project_id",projectId).maybeSingle();if(project.data&&client.data){const plan=planEntitlements(planKey),serviceMode=planKey==="pro"||planKey==="autopilot_plus"?"managed_agent":"copilot";const enrollment=await db.from("agent_service_enrollments").upsert({agency_id:project.data.agency_id,client_organization_id:project.data.client_organization_id,client_id:client.data.id,project_id:projectId,service_mode:serviceMode,operator_brand:"hdseo",approval_owner:"client",billing_owner:"client",plan_key:planKey,status:"active",monthly_action_limit:plan.monthlyActionLimit,monthly_major_page_limit:plan.monthlyMajorPageLimit,monthly_provider_budget:plan.monthlyProviderBudget,monthly_human_review_minutes:plan.humanReviewMinutes,cycle_cadence_hours:plan.cycleCadenceHours,next_cycle_at:new Date().toISOString(),stripe_customer_id:object.customer,stripe_subscription_id:object.subscription,subscription_id:subscription.data?.id??null,updated_at:new Date().toISOString()},{onConflict:"project_id"});if(enrollment.error)throw new ApiError("The paid service entitlement could not be activated. Apply migration 0034.",503,"DATABASE_BINDING_FAILED");}}
      }
