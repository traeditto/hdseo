...
const schema = z.object({ agencyId: z.string().uuid(), clientId: z.string().uuid(), projectId: z.string().uuid(), confirmationId: z.string().uuid(), keywords: z.array(z.string().min(2)).max(700).optional(), target: z.string().min(3).optional(), limit: z.number().int().min(1).max(1000).default(100), locationCode: z.number().int().positive().default(2840), locationName: z.string().optional(), languageCode: z.string().length(2).default("en") });
...
export async function POST(request: Request, { params }: { params: Promise<{ operation: string }> }) {
  ...
  try {
    ...
    const scope={operation,keywords:input.keywords??null,target:input.target??null,limit:input.limit,locationCode:input.locationCode,languageCode:input.languageCode};
