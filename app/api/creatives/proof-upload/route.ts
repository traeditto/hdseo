...
import{addBusinessProof}from"@/lib/creatives/service";
...
export async function POST(request:Request){try{
  ...
  const context=await requireLiveAgencyProject({projectId,permission:"seo.write"});await enforceRateLimit(`agency:${context.agencyId}:project:${projectId}`,"proof_upload",30,3600);
  ...
  await auditEvent({agencyId:context.agencyId,actorUserId:context.userId,action:"creative.proof_uploaded",resourceType:"business_proof_asset",resourceId:proof.id,request,afterState:{projectId,mimeType:file.type,size:file.size}});return Response.json({ok:true,proof},{status:201});
