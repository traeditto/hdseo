...
export async function POST(request:Request){
  try{
    const input=await parseJson(request,schema),permission=input.action==="decide"?"execution.approve":"seo.write",context=await requireLiveAgencyProject({projectId:input.projectId,permission});
