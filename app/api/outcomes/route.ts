...
const requiredBudgetCategories=["data","content","technical","local","authority","implementation","software","reserve"] as const;
const budgetAllocations=z.array(z.object({category:categories,monthlyAmount:z.number().min(0),approvalThreshold:z.number().min(0).default(0)})).length(8).superRefine((items,context)=>{
  const seen=new Set(items.map(item=>item.category));
  if(requiredBudgetCategories.some(category=>!seen.has(category)))context.addIssue({code:"custom",message:"Provide one allocation for every SEO budget category."});
});
const schema=z.discriminatedUnion("action",[
  z.object({action:z.literal("configure_budget"),projectId:z.string().uuid(),monthlyLimit:z.number().min(0).max(10_000_000),warningPercent:z.number().min(1).max(100).default(80),hardStop:z.boolean().default(true),allocations:budgetAllocations}),
