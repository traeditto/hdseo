...
export async function GET(request:Request){
  ...
  try{
    const settled=await Promise.allSettled([processDeploymentBatch(),processAgentBatch(env.AUTOMATION_JOB_BATCH_SIZE),syncOutcomeProviders()]),names=["deployments","agents","outcomes"] as const,workers=Object.fromEntries(settled.map((result,index)=>[names[index],result.status==="fulfilled"?{ok:true,result:result.value}:{ok:false,error:result.reason instanceof Error?result.reason.name:"WORKER_FAILED"}])),failures=settled.flatMap((result,index)=>result.status==="rejected"?[names[index]]:[]),deployments=settled[0].status==="fulfilled"?settled[0].value:null,agents=settled[1].status==="fulfilled"?settled[1].value:null,outcomes=settled[2].status==="fulfilled"?settled[2].value:null;
    await recordSystemHeartbeat({component:"scheduler:automation",status:failures.length?"degraded":"healthy",workerId,metadata:{phase:failures.length?"partial":"completed",failures,deploymentsClaimed:deployments?.claimed??0,agentsClaimed:agents?.claimed??0,outcomesClaimed:outcomes?.claimed??0}});
    return Response.json({ok:failures.length===0,workers,timestamp:new Date().toISOString()},{status:failures.length?207:200});
  }catch(error){
