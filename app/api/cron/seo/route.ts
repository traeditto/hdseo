...
export async function GET(request:Request){
  ...
  try{
    ...
    await recordSystemHeartbeat({component:"scheduler:seo",status:degraded?"degraded":"healthy",workerId,metadata:{phase:"completed",evidenceClaimed:evidence.claimed??0,campaignClaimed:Array.isArray(jobs)?jobs.length:0,monitoringProcessed:checkpoints.filter(item=>item.status!=="idle").length}});
    return Response.json({ok:!degraded,evidence,jobs,checkpoints,timestamp:new Date().toISOString()},{status:degraded?207:200});
  }catch(error){
