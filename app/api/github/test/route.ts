...
export async function POST(request:Request){try{
  ...
  const[installation,repositories]=await Promise.all([getInstallation(Number(record.data.installation_id)),listInstallationRepositories(Number(record.data.installation_id))]),now=new Date().toISOString();
  await context.db.from("github_installations").update({account_login:installation.account.login,repository_selection:installation.repository_selection,permissions:installation.permissions,events:installation.events,last_synced_at:now,updated_at:now}).eq("id",record.data.id);
