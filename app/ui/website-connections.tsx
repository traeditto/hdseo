...
type Website = {
  ...
  googleSearchConsole:{id:string;status:string;selectedProperty:string|null;lastSyncedAt:string|null;lastVerifiedAt:string|null;properties:SearchConsoleProperty[];health:string}|null;
};
...
export function WebsiteConnections({agencyId,projects,websites,canManage,busy,onAction,onOpenPackages,openSetupProjectId}:{agencyId:string;projects:Project[];websites:Website[];canManage:boolean;busy:boolean;onAction:(body:Record<string,unknown>)=>Promise<boolean>;onOpenPackages:()=>void;openSetupProjectId?:string|null}){
  ...
  async function refreshProperties(project:string){setIntegrationBusy("properties");setIntegrationMessage("");try{const response=await fetch("/api/google/properties",{method:"PUT",headers:{"content-type":"application/json"},body:JSON.stringify({projectId:project})}),result=await response.json();if(!response.ok){setIntegrationMessage(result.error?.message??"Authorized properties could not be refreshed.");return;}setPropertyOptions(current=>({...current,[project]:result.properties??[]}));setIntegrationMessage(result.message??"Authorized properties refreshed.");if(result.selectedProperty)window.location.reload();}catch{setIntegrationMessage("Authorized properties could not be reached. Try again.");}finally{setIntegrationBusy(null);}}
  async function chooseProperty(project:string,property:string){setIntegrationBusy("property");setIntegrationMessage("");try{const response=await fetch("/api/google/properties",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({projectId:project,property})}),result=await response.json();if(!response.ok){setIntegrationMessage(result.error?.message??"The Search Console property could not be selected.");return;}window.location.reload();}catch{setIntegrationMessage("The Search Console property could not be reached. Try again.");}finally{setIntegrationBusy(null);}}
