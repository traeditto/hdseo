...
const navigation: { label?: string; items: NavItem[] }[] = [
  ...
  { label: "OPERATIONS", items: [{ label: "Implementation packages", glyph: "▧", count: 3 }, { label: "Executions", glyph: "⌘" }, { label: "Approvals", glyph: "◉", count: 4 }, { label: "Monitoring", glyph: "◷", count: 9 }, { label: "Team", glyph: "♙" }, { label: "Integrations", glyph: "⇄" }] },
];
...
const views: Record<string, { eyebrow: string; title: string; description: string }> = {
  ...
  "Implementation packages": { eyebrow: "MANUAL DELIVERY", title: "Implementation packages", description: "WordPress, CMS-neutral, and developer-ready work built from approved evidence." },
  ...
  Monitoring: { eyebrow: "OUTCOME TRACKING", title: "7/14/30/60/90 monitoring", description: "Verified implementations and their scheduled evidence checkpoints." },
...
function ProductView({ active, view, clients, queue, priorityFilter, setPriorityFilter, open, go }: { active: string; view: { eyebrow: string; title: string; description: string }; clients: Client[]; queue: QueueItem[]; priorityFilter: string; setPriorityFilter: (value: string) => void; open: (kind: string, title: string, description: string, subject?: string) => void; go: (view: string) => void }) {
  return <><div className="view-hero"><span>{view.eyebrow}</span><h1>{view.title}</h1><p>{view.description}</p></div>{active === "Next best actions" && <div className="opportunity-grid">{opportunities.map((item) => <article className="opportunity-card" key={item.action}><div className="op-score">{item.score}</div><div><span>{item.type} · {item.client}</span><h2>{item.action}</h2><p>Current {item.rank} · Target {item.target} · {item.value}</p></div><div className="card-actions"><button onClick={() => open("evidence", item.action, "Review scoring contributions, ranking history, and competitor evidence.", item.client)}>Evidence</button><button onClick={() => open("draft", "Create implementation draft", "Prepare a human-reviewable execution brief.", item.action)}>Create draft</button></div></article>)}</div>}
    ...
    {active === "Implementation packages" && <CardList items={[{ title: "Roof Repair Jacksonville", meta: "WordPress · Gutenberg + Yoast fields · Verified evidence only", status: "AGENCY REVIEW" }, { title: "Ponte Vedra AC Repair", meta: "WordPress · Elementor handoff · Client approval requested", status: "CLIENT" }, { title: "Canonical conflict repair", meta: "Developer ticket · Acceptance and rollback criteria included", status: "READY" }]} action="Open package" kind="package" open={open} secondary="Send to client" />}
    {active === "Executions" && <CardList items={[{ title: "Roof repair WordPress package", meta: "Implemented · Live verification required before monitoring", status: "VERIFY" }, { title: "Ponte Vedra service page", meta: "Manual implementation · Awaiting strategist", status: "REVIEW" }, { title: "Repository execution", meta: "Disabled until the manual-workflow readiness gate passes", status: "LOCKED" }]} action="Review execution" kind="execution" open={open} />}
    ...
    {active === "Monitoring" && <CardList items={[{ title: "Emergency roof repair improvement", meta: "Day 14 · Position #6 → #4 · Movement observed, causation not claimed", status: "ON TRACK" }, { title: "Jacksonville service metadata", meta: "Day 7 checkpoint due tomorrow · Fresh ranking collection required", status: "DUE" }, { title: "Canonical conflict repair", meta: "Indexability check inconclusive · Retry scheduled", status: "REVIEW" }]} action="Open checkpoints" kind="monitoring" open={open} />}
    ...
    {active === "Integrations" && <CardList items={[{ title: "HD SEO Automation Engine", meta: "Durable jobs · Evidence gates · Human approval · Outcome monitoring", status: "BUILT" }, { title: "DataForSEO", meta: "Platform and agency credential architecture · Explicit paid authorization", status: "SETUP" }, { title: "Google Search Console", meta: "OAuth provider foundation · Credentials required", status: "SETUP" }, { title: "GitHub App", meta: "Feature-disabled until a complete manual workflow is verified", status: "LOCKED" }, { title: "Stripe", meta: "Server-side entitlement foundation · Credentials required", status: "SETUP" }, { title: "Resend", meta: "Transactional email credentials required", status: "SETUP" }]} action="Configure" kind="integration" open={open} />}
  </>;
}
...
function WorkflowModal({ modal, close, finish, addClient, searchTerm, setSearchTerm, clients, notifications, clearNotifications, completeQueueItem }: { modal: NonNullable<ModalState>; close: () => void; finish: (message: string) => void; addClient: (event: FormEvent<HTMLFormElement>) => void; searchTerm: string; setSearchTerm: (value: string) => void; clients: Client[]; notifications: number; clearNotifications: () => void; completeQueueItem: (title: string, verb: string) => void }) {
  ...
  return <div className="modal-backdrop" role="presentation" onMouseDown={close}><div className="modal workflow-modal" role="dialog" aria-modal="true" aria-labelledby="modal-title" onMouseDown={(event) => event.stopPropagation()}><button className="modal-close" onClick={close} aria-label="Close">×</button><span className="modal-kicker">HD SEO WORKFLOW</span><h2 id="modal-title">{modal.title}</h2><p>{modal.description}</p>
    ...
    {modal.kind === "draft" && <form className="workflow-form" onSubmit={(event) => { event.preventDefault(); finish("Implementation package saved for agency review"); }}><div className="evidence-summary"><span>OPPORTUNITY</span><strong>{modal.subject}</strong><p>Score 94 · Confidence 86% · Target Top 3</p></div><Field label="Implementation path"><select><option>WordPress implementation package</option><option>Generic CMS package</option><option>Developer-ready ticket</option><option disabled>Repository execution — readiness gate locked</option></select></Field><Field label="Assigned strategist"><select><option>Maya Chen</option><option>Olivia Martin</option></select></Field><Field label="Draft notes"><textarea defaultValue="Strengthen service coverage, add four contextual internal links, and align metadata with verified business evidence." /></Field><div className="modal-actions"><button type="button" onClick={close}>Cancel</button><button type="submit">Create package</button></div></form>}
