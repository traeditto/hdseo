...
export function PortalLogin({portal,authMode}:{portal:PortalRole;authMode:"supabase"|"chatgpt"}){
  ...
  return <main className={`portal-login-page portal-login-${portal}`}>
    ...
    <section className="portal-login-form"><div className="login-form-wrap"><Link className="back-link" href="/login">← All portals</Link><PortalRoleSelector activeRole={portal} /><span className="form-eyebrow">{copy.eyebrow}</span><h2>{status==="recovery"?"Reset your password":`Sign in to ${copy.title}`}</h2><p>{status==="recovery"?"Enter your account email and we’ll send a secure recovery link.":copy.description}</p>
