...
export function PortalLogin({portal,authMode}:{portal:PortalRole;authMode:"supabase"|"chatgpt"}){
  ...
  async function signup(event:FormEvent<HTMLFormElement>){
    ...
    const result=await db.auth.signUp({email,password,options:{data:{full_name:fullName},emailRedirectTo:`${window.location.origin}/auth/callback?next=/portal/agency`}});
    if(result.error){setMessage(result.error.message);return;}
    if(result.data.session){router.push("/portal/agency");router.refresh();return;}
    setMessage("Check your email to verify the account, then return here to create your agency workspace.");
  }
  ...
  return <main className={`portal-login-page portal-login-${portal}`}>
    ...
    <section className="portal-login-form"><div className="login-form-wrap"><Link className="back-link" href="/">← All portals</Link><span className="form-eyebrow">{copy.eyebrow}</span><h2>{status==="recovery"?"Reset your password":`Sign in to ${copy.title}`}</h2><p>{status==="recovery"?"Enter your account email and we’ll send a secure recovery link.":copy.description}</p>
      ...
      :<form onSubmit={signup}><label>Your name<input name="fullName" autoComplete="name" required/></label><label>Work email<input name="signupEmail" type="email" autoComplete="email" placeholder="you@agency.com" required/></label><label>Create password<input name="signupPassword" type="password" autoComplete="new-password" minLength={10} required/></label>{message&&<div className="login-message" role="status">{message}</div>}<button className="login-submit" type="submit">Create agency account <span>→</span></button><button className="recovery-back" type="button" onClick={()=>{setStatus("idle");setMessage("");}}>Back to sign in</button></form>}
