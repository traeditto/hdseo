...
import {foundingBetaProgram} from "@/lib/billing/catalog";
...
type OfferMode="beta"|"standard";

function offerHref(href:string,offer:OfferMode){
  if(offer!=="beta")return href;
  const separator=href.includes("?")?"&":"?";
  return `${href}${separator}offer=${foundingBetaProgram.offerKey}`;
}
...
export default function PricingExperience() {
  ...
  const [offerMode,setOfferMode]=useState<OfferMode>("beta");
  ...
  const planCost = offerMode==="beta"?activePlan.betaOffer.price:billing === "annual" && activeAnnual ? activeAnnual / 12 : activePlan.monthly;
  ...
  const changeBilling = (next: string) => {
    ...
    setBilling(typed);
    if(typed==="annual")setOfferMode("standard");
    emitPricingEvent("pricing_billing_toggle", { billing: typed });
  };
  const changeOffer=(next:string)=>{
    const typed=next as OfferMode;
    setOfferMode(typed);
    if(typed==="beta")setBilling("monthly");
    emitPricingEvent("pricing_offer_toggle",{offer:typed});
  };
  ...
  return <main className="marketing-site pricing-site"><a className="skip-link" href="#pricing-main">Skip to pricing</a><MarketingHeader /><div id="pricing-main">
    <section className="pricing-hero"><WorkModeSelector value={mode} onChange={changeMode} /><p className="mode-summary">{mode === "guide" ? "Choose the platform capacity that fits your team, then manage the approval workflow yourself." : audience === "business" ? "Meet HD SEO Autopilot: your agent-operated SEO department, bounded by your approvals and budget." : "Add a White-Label Agent Team that delivers approved client work without adding payroll."}</p><div className="beta-launch-banner"><strong>FOUNDING BETA IS OPEN</strong><span>Real production workflows, limited enrollment, and the first billing period at the beta price.</span><small>The first {foundingBetaProgram.measurementWindowDays} days are measured as a learning period. HD SEO prioritizes completed, verifiable outcomes—not agent chatter or busywork. The plan renews at the clearly shown standard monthly price unless canceled. Rankings and revenue are never guaranteed.</small></div><div className="pricing-controls"><SegmentedControl label="Choose your audience" value={audience} onChange={changeAudience} options={[{ value: "business", label: "Business" }, { value: "agency", label: "Agency" }]} /><SegmentedControl label="Choose offer" value={offerMode} onChange={changeOffer} options={[{value:"beta",label:"Founding Beta"},{value:"standard",label:"Standard pricing"}]} />{offerMode==="standard"&&<SegmentedControl label="Billing frequency" value={billing} onChange={changeBilling} options={[{ value: "monthly", label: "Monthly" }, { value: "annual", label: "Annual — 2 months free" }]} />}</div><div className="pricing-hero-links"><Link href="/register" data-analytics-event="free_trial_signup_click" data-analytics-placement="pricing_hero">Start free — one website crawl</Link><Link href="/audit" data-analytics-event="primary_audit_cta_click" data-analytics-placement="pricing_hero">Audit without an account</Link><Link href="/login/client">Existing business sign in</Link><Link href="/login/agency">Agency sign in</Link></div></section>

    {mode === "guide" ? <section className="pricing-card-section" aria-live="polite"><header><span className="m-eyebrow">PLATFORM MODE · {audience === "business" ? "BUSINESS PLANS" : "AGENCY PLANS"}</span><h2>{audience === "business" ? "From foundational fixes to multi-location growth." : "Pay for active client websites—not seats."}</h2><p>{audience === "business" ? "All plans combine research, approval-controlled implementation, validation, and outcome reporting." : "Add unlimited team members and read-only client viewers without per-seat charges."}</p></header><div className="pricing-grid">{platformPlans.map(plan => {const beta=offerMode==="beta",display=beta?plan.betaOffer.price:billing === "monthly" ? plan.monthly : plan.annual;return <article id={plan.slug} key={plan.slug} className={plan.label ? "recommended" : ""} tabIndex={0} aria-label={`${plan.name} plan, ${formatUsd(display)}`}>
      {plan.label && <strong className="pricing-badge">{plan.label}</strong>}<span className="plan-audience">{beta?"FOUNDING BETA":plan.audience === "agency" ? "AGENCY" : "BUSINESS"}</span><h2>{plan.name}</h2><div className="plan-price"><strong>{formatUsd(display)}</strong><span>{beta ? "/first month" : billing === "monthly" ? "/month" : "/year"}</span></div>{beta?<small className="beta-renewal"><del>{formatUsd(plan.monthly)}/month</del><b>Then {formatUsd(plan.betaOffer.renewalPrice)}/month unless canceled</b><span>{plan.betaOffer.enrollmentLimit} founding spots allocated to this tier</span></small>:billing === "annual" && <small className="annual-note">Equivalent to {formatUsd(plan.annual / 12)}/month · 2 months free</small>}<p>{plan.description}</p><Link href={offerHref(plan.href,offerMode)} data-analytics-event={plan.audience === "agency" ? "agency_plan_selection" : "business_plan_selection"} data-analytics-placement={plan.slug}>{beta?`Start ${plan.name} Beta`:plan.cta}<Arrow /></Link>{plan.supportingText && <small className="plan-support">{plan.supportingText}</small>}<ul>{plan.features.map(feature => <li key={feature}>{feature}</li>)}</ul>
    </article>})}</div></section> : <section className="pricing-card-section agent-service-section" aria-live="polite"><header><span className="m-eyebrow">AGENT SERVICE MODE</span><h2>{audience === "business" ? "HD SEO Autopilot — Run It For Me" : "White-Label Agent Team"}</h2><p>{audience === "business" ? "HD SEO agents find, prepare, validate, publish, and monitor approved work. You primarily review clear business decisions." : "Add an autonomous, white-label SEO delivery team without adding payroll."}</p></header><div className={`pricing-grid agent-service-grid ${audience}`}>{servicePlans.map(plan => {
      const annualPrice = plan.annual;
      const beta=offerMode==="beta",showAnnual = !beta&&billing === "annual" && annualPrice,display=beta?plan.betaOffer.price:showAnnual?annualPrice:plan.monthly;
      return <article id={plan.slug} key={plan.slug} className="recommended" tabIndex={0} aria-label={`${plan.name}, ${formatUsd(display)}`}><strong className="pricing-badge">{beta?"RUN IT FOR ME · FOUNDING BETA":plan.label}</strong><span className="plan-audience">{audience === "business" ? "HD SEO AGENT TEAM" : "WHITE-LABEL AGENT TEAM"}</span><h2>{plan.name}</h2><div className="plan-price"><strong>{formatUsd(display)}</strong><span>{beta?`/${plan.betaOffer.durationDays} days`:showAnnual ? "/year" : audience === "business" ? "/month" : ""}</span></div>{beta?<small className="beta-renewal"><del>{formatUsd(plan.monthly)}/month</del><b>Then {formatUsd(plan.betaOffer.renewalPrice)}/month unless canceled</b><span>{plan.betaOffer.enrollmentLimit} founding spots allocated to this tier</span></small>:<small className="annual-note">{showAnnual ? "Two months free" : plan.priceQualifier}</small>}{plan.foundingOffer&&!beta && <strong className="founding-offer">{audience === "business" ? "Founding pilot: " : ""}{plan.foundingOffer}</strong>}<p>{plan.description}</p><Link href={offerHref(plan.href,offerMode)} data-analytics-event={audience === "agency" ? "agency_plan_selection" : "business_plan_selection"} data-analytics-placement={plan.slug}>{beta?audience==="business"?"Start Run It For Me Beta":"Apply for Founding Beta":plan.cta}<Arrow /></Link><ul>{plan.features.map(feature => <li key={feature}>{feature}</li>)}</ul></article>;
  ...
  </div><Link className="pricing-mobile-cta" href={offerHref(stickyPlan.href,offerMode)} data-analytics-event={audience === "business" ? "business_plan_selection" : "agency_plan_selection"} data-analytics-placement="mobile_sticky">{offerMode==="beta"?mode==="agent-service"?"Start Run It For Me Beta":"Start Founding Beta":mode === "agent-service" ? stickyPlan.cta : audience === "business" ? "Choose Growth" : "Choose Agency Growth"} <Arrow /></Link><MarketingFooter /></main>;
}
