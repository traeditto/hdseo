...
describe("customer-visible accountable work receipts", () => {
  ...
  it("refreshes active receipts automatically without requiring SEO or queue knowledge", () => {
    ...
    expect(route).toContain("continuesAutomatically");
    expect(route).toContain("workerCadenceMinutes: continuesAutomatically ? 1 : null");
    expect(receipt).toContain("Automatic—no action needed");
    expect(receipt).toContain("Last recorded progress");
  });
