...
describe("Founding Beta",()=>{
  it("offers every direct business tier with a bounded 10-15% modeled margin",()=>{
    ...
    for(const plan of Object.values(retailBillingPlans)){
      ...
      expect(contributionMarginPercent(plan.beta.priceCents,plan.beta.maxAllInCostCents)).toBeGreaterThanOrEqual(10);
      expect(contributionMarginPercent(plan.beta.priceCents,plan.beta.maxAllInCostCents)).toBeLessThanOrEqual(15);
    }
  });
