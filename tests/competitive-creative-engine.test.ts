...
describe("competitive creative engine",()=>{
  ...
  it("lets low-risk metadata alignment prepare itself without weakening full-content proof gates",()=>{const service=read("lib/creatives/service.ts");expect(service).toContain('metadataOnly=opportunity.action_type==="IMPROVE"');expect(service).toContain('mode:"metadata_only"');expect(service).toContain('if(!metadataOnly&&usableProof.length<2)');expect(service).toContain('model:"hdseo-metadata-v1"');expect(service).toContain("No new factual business claims");expect(service).toContain("autoApprovedPreparation:autoApproved");expect(service).toContain('if(draft.status==="approved")return{state:"approved"');});
