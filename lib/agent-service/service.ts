...
export async function agentServiceSnapshot(db:SupabaseClient,tenant:Omit<AgentServiceTenant,"userId">){
  ...
  const [cycles,usage,escalations,client,outcomeRuns,campaignDecisions,executionDecisions,releaseDecisions]=await Promise.all([
    ...
    db.from("outcome_loop_runs").select("id,opportunity_id,status,current_step,expected_value,delivery_kind,delivered_at,campaign_job_id,execution_id,monitoring_plan_id,failure_code,failure_message,created_at,updated_at").eq("enrollment_id",enrollment.data.id).order("created_at",{ascending:false}).limit(30),
