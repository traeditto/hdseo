...
async function beginCycle(db:SupabaseClient,enrollment:Enrollment){
  ...
  for(const [index,workType] of workTypes.entries())results.push(await enqueueAgentWorkItem(db,{agencyId:enrollment.agency_id,clientId:enrollment.client_organization_id,projectId:enrollment.project_id,userId:enrollment.created_by},{workType,evidence:{cycleId:sourceId,opportunity:opportunity.data},proposedPlan:{serviceMode:"managed_agent",approvalOwner:enrollment.approval_owner,noMakeWorkRule:true},spendingLimit:workType==="research.discovery"?providerCost:0,priority:95-index*10,idempotencyKey:`agent-service:${sourceId}:${workType}`,sourceType:"agent_service_cycle",sourceId,approvalOwner:enrollment.approval_owner}));
