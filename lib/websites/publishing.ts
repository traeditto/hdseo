...
async function rollbackWordPress(context:ProviderContext,resourceId:string,before:Record<string,unknown>):Promise<ProviderWrite>{
  const current=await wordpressPage(context,resourceId),base=normalizeSiteUrl(context.siteUrl).siteUrl,body={title:String(before.title??""),content:String(before.content??""),excerpt:String(before.excerpt??""),status:String(before.status??current.status)};
  ...
  return{resourceType:"page",resourceId,before:wordpressSnapshot(current),after:wordpressSnapshot(restored),result:{httpStatus:response.status,providerRevision:restored.modified_gmt??null}};
}
...
async function rollbackShopify(context:ProviderContext,resourceId:string,before:Record<string,unknown>):Promise<ProviderWrite>{
  const current=await shopifyPage(context,resourceId),restored=await mutateShopifyPage(context,resourceId,{title:String(before.title??""),body:String(before.body??""),isPublished:Boolean(before.isPublished)});
  return{resourceType:"page",resourceId,before:shopifySnapshot(current),after:shopifySnapshot(restored.page),result:{httpStatus:restored.httpStatus,providerRevision:restored.page.updatedAt}};
}
...
async function rollbackWebflow(context:ProviderContext,resourceId:string,before:Record<string,unknown>):Promise<ProviderWrite>{
  const current=await webflowPage(context,resourceId),restored=await mutateWebflowMetadata(context,resourceId,{seo:before.seo??{},openGraph:before.openGraph??{},localeId:before.localeId??null}),publishStatus=await publishWebflowPage(context,resourceId);
  return{resourceType:"page",resourceId,before:webflowSnapshot(current),after:webflowSnapshot(restored.page),result:{httpStatus:restored.httpStatus,publishHttpStatus:publishStatus,providerRevision:restored.page.lastUpdated??null}};
}
...
export async function publishCmsPackage(db:SupabaseClient,input:{packageId:string;agencyId:string;projectId:string;actorId:string;idempotencyKey:string}){
  ...
  const existing=await db.from("cms_publications").select("id,status,provider_resource_id,target_url").eq("agency_id",input.agencyId).eq("idempotency_key",input.idempotencyKey).maybeSingle();if(existing.data&&["publishing","published","rolled_back"].includes(existing.data.status))return existing.data;
  ...
  try{
    const write=provider==="wordpress"?await publishWordPress(context):provider==="shopify"?await publishShopify(context):await publishWebflow(context),now=new Date().toISOString(),saved=await db.from("cms_publications").update({status:"published",provider_resource_type:write.resourceType,provider_resource_id:write.resourceId,before_snapshot:write.before,after_snapshot:write.after,provider_result:write.result,published_at:now,updated_at:now}).eq("id",ledgerId).select("id,status,provider,provider_resource_id,target_url,published_at").single();
    ...
    await db.from("implementation_packages").update({status:"implemented_unverified",implemented_at:now,updated_at:now}).eq("id",pkg.id);await db.from("implementation_verifications").upsert({agency_id:pkg.agency_id,client_organization_id:pkg.client_organization_id,project_id:pkg.project_id,package_id:pkg.id,live_url:payload.targetUrl,status:"pending",proof:{publicationId:ledgerId,provider},checks:{},error_details:{}},{onConflict:"package_id"});await db.from("proof_of_work_events").insert({agency_id:pkg.agency_id,client_organization_id:pkg.client_organization_id,project_id:pkg.project_id,opportunity_id:pkg.opportunity_id,package_id:pkg.id,event_type:"cms_published",title:`${provider} change published`,description:"The approved package was published through the verified provider connection and is awaiting live QA.",client_visible:true,actor_user_id:input.actorId,metadata:{publicationId:ledgerId,provider,resourceId:write.resourceId}});await audit(db,pkg,input.actorId,"cms.publication.published",ledgerId,{provider,packageId:pkg.id,projectId:pkg.project_id});return saved.data;
  }catch(error){const code=error instanceof ApiError?error.code:"OPERATION_FAILED";await db.from("cms_publications").update({status:"publish_failed",error_code:code,error_message:error instanceof ApiError?error.message:"Provider publishing failed.",updated_at:new Date().toISOString()}).eq("id",ledgerId);logServerError("cms_publish_failed",error,{agencyId:input.agencyId,projectId:input.projectId,provider,operation:"publish"});throw error;}
}
...
export async function rollbackCmsPublication(db:SupabaseClient,input:{publicationId:string;agencyId:string;projectId:string;actorId:string}){
  ...
  const provider=connection.data.cms_type as Provider,secret=JSON.parse(decryptSecret(connection.data.encrypted_secret_reference)) as ConnectionSecret,context:ProviderContext={provider,siteUrl:connection.data.site_url,secret,payload:{targetUrl:String(publication.target_url),providerResourceId:String(publication.provider_resource_id)}};await db.from("cms_publications").update({status:"rolling_back",updated_at:new Date().toISOString()}).eq("id",input.publicationId);
  try{const before=publication.before_snapshot as Record<string,unknown>,write=provider==="wordpress"?await rollbackWordPress(context,String(publication.provider_resource_id),before):provider==="shopify"?await rollbackShopify(context,String(publication.provider_resource_id),before):await rollbackWebflow(context,String(publication.provider_resource_id),before),now=new Date().toISOString(),saved=await db.from("cms_publications").update({status:"rolled_back",provider_result:{...((publication.provider_result&&typeof publication.provider_result==="object")?publication.provider_result:{}),rollback:write.result},rolled_back_by:input.actorId,rolled_back_at:now,updated_at:now}).eq("id",input.publicationId).select("id,status,provider,target_url,rolled_back_at").single();if(!saved.data)throw new ApiError("Rollback succeeded but its ledger could not be finalized.",500,"DATABASE_BINDING_FAILED");const pkg={agency_id:publication.agency_id,project_id:publication.project_id};await audit(db,pkg,input.actorId,"cms.publication.rolled_back",input.publicationId,{provider,packageId:publication.package_id,projectId:publication.project_id});return saved.data;}catch(error){const code=error instanceof ApiError?error.code:"OPERATION_FAILED";await db.from("cms_publications").update({status:"rollback_failed",error_code:code,error_message:error instanceof ApiError?error.message:"Provider rollback failed.",updated_at:new Date().toISOString()}).eq("id",input.publicationId);logServerError("cms_rollback_failed",error,{agencyId:input.agencyId,projectId:input.projectId,provider,operation:"rollback"});throw error;}
}
