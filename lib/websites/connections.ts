...
export type ConnectWebsiteInput={projectId:string;mode:WebsiteConnectionMode;siteUrl:string;username?:string;applicationPassword?:string;accessToken?:string;siteId?:string;platformName?:string;notes?:string};
...
async function connectionContext(email:string,projectId?:string,websiteId?:string){
  ...
  const membership=await db.from("agency_members").select("agency_id,role").eq("user_id",profile.data.id).eq("status","active").limit(1).maybeSingle();
  if(!membership.data)throw new ApiError("Agency access denied.",403,"TENANT_DENIED");
  if(!hasPermission(membership.data.role as AgencyRole,"integrations.manage"))throw new ApiError("Your agency role cannot manage website connections.",403,"ROLE_FORBIDDEN");
  let projectQuery=db.from("seo_projects").select("id,name,domain,client_organization_id").eq("agency_id",membership.data.agency_id).eq("status","active");
  if(projectId)projectQuery=projectQuery.eq("id",projectId);
  if(websiteId){const website=await db.from("websites").select("project_id").eq("id",websiteId).eq("agency_id",membership.data.agency_id).maybeSingle();if(!website.data)throw new ApiError("Website connection not found.",404,"NOT_FOUND");projectQuery=projectQuery.eq("id",website.data.project_id);}
  const project=(await projectQuery.limit(1)).data?.[0];if(!project)throw new ApiError("Client project not found.",404,"NOT_FOUND");
  return{db,userId:profile.data.id,agencyId:membership.data.agency_id,project};
}
...
export async function connectWebsite(email:string,input:ConnectWebsiteInput){
  const context=await connectionContext(email,input.projectId),values=modeValues(input),verified=await verifyDirect(input.mode,input.siteUrl,values.secret??({version:1,provider:"wordpress"} as ConnectionSecret)),now=new Date().toISOString();
  const website=await context.db.from("websites").upsert({agency_id:context.agencyId,client_organization_id:context.project.client_organization_id,project_id:context.project.id,name:context.project.name,site_url:verified.siteUrl,canonical_domain:verified.canonicalDomain,cms_type:values.cmsType,is_primary:true,status:values.status,last_verified_at:values.status==="pending"?null:now,updated_at:now},{onConflict:"project_id,canonical_domain"}).select("id").single();
  ...
  return{websiteId:website.data.id,status:values.status,cmsType:values.cmsType};
}
...
export async function testWebsiteConnection(email:string,websiteId:string){
  ...
  if(!["wordpress","shopify","webflow"].includes(mode)){await assertPublicSiteUrl(website.data.site_url);await context.db.from("websites").update({last_verified_at:new Date().toISOString(),updated_at:new Date().toISOString()}).eq("id",websiteId);return{status:connection.data.status,message:"The public website is reachable."};}
  ...
  return{status:"active",message:"Provider credentials and website access were verified."};
}
