...
type BackgroundJob={id:string;agency_id:string;payload:Record<string,unknown>;attempt_count:number;max_attempts:number};
...
async function toolExecution(db:SupabaseClient,work:WorkItem,toolKey:string,status:"succeeded"|"failed"|"denied",summary:Record<string,unknown>,cost=0){
  ...
  const risk=await db.from("agent_tools").select("default_risk_level").eq("tool_key",toolKey).maybeSingle();
  const result=await db.from("agent_tool_executions").upsert({agency_id:work.agency_id,client_id:work.client_id,project_id:work.project_id,work_item_id:work.id,agent_key:work.assigned_agent_key,tool_key:toolKey,status,risk_level:risk.data?.default_risk_level??"low",request_payload:{goal:work.goal},response_summary:summary,cost_amount:cost,idempotency_key:`${toolKey}:${work.id}`,started_at:now(),completed_at:now()},{onConflict:"work_item_id,idempotency_key"});
  if(result.error)throw new ApiError("The agent tool execution could not be audited.",500,"DATABASE_BINDING_FAILED");
  if(cost>0)await db.from("agent_work_items").update({spent_amount:num(work.spent_amount)+cost,updated_at:now()}).eq("id",work.id);
}
...
const stepPlans:Record<string,Array<{agentKey?:string;stepType:string;title:string;toolKey?:string}>>={
  ...
  "strategy.roadmap":[{stepType:"evidence.collect",title:"Read approved opportunities",toolKey:"audit.read"},{stepType:"plan",title:"Build the 30, 60, and 90-day roadmap",toolKey:"strategy.plan"},{agentKey:"qa",stepType:"validate",title:"Validate dependencies and expected value"}],
  "technical.audit":[{stepType:"evidence.collect",title:"Collect crawl and indexing evidence",toolKey:"website.crawl"},{stepType:"analyze",title:"Analyze indexability, metadata, schema, links, and performance",toolKey:"seo.validate"},{agentKey:"qa",stepType:"validate",title:"Validate technical findings",toolKey:"robots.verify"}],
  "content.plan":[{stepType:"evidence.collect",title:"Read search, page, and verified business proof",toolKey:"proof.read"},{stepType:"draft",title:"Compile intent-specific creative specifications",toolKey:"creative.spec"},{agentKey:"qa",stepType:"validate",title:"Validate claims, originality requirements, and target intent"}],
  ...
  "qa.validate":[{stepType:"validate",title:"Run deployment and SEO validation",toolKey:"seo.validate"},{stepType:"rollback.check",title:"Confirm rollback readiness",toolKey:"vercel.rollback"}],
  "reporting.summary":[{stepType:"evidence.collect",title:"Collect outcomes, spend, and proof of work",toolKey:"audit.read"},{stepType:"report",title:"Prepare plain-language value summary",toolKey:"report.generate"}],
};
...
async function enforceApprovals(db:SupabaseClient,work:WorkItem){
  ...
  const existing=await db.from("agent_approvals").select("approval_type,status").eq("work_item_id",work.id);
  const status=new Map((existing.data??[]).map(row=>[row.approval_type,row.status]));
  if(requirements.some(requirement=>status.get(requirement.type)==="rejected")){await db.from("agent_work_items").update({status:"blocked",final_outcome:{reason:"Required approval was rejected."},updated_at:now()}).eq("id",work.id);return false;}
  const missing=requirements.filter(requirement=>status.get(requirement.type)!=="approved");
  for(const requirement of missing){if(!requirement.type||status.has(requirement.type))continue;await db.from("agent_approvals").insert({agency_id:work.agency_id,client_id:work.client_id,project_id:work.project_id,work_item_id:work.id,approval_type:requirement.type,title:`Approve ${work.assigned_agent_key.replaceAll("_"," ")} work`,summary:requirement.reason??work.goal,risk_level:work.risk_level,requested_decision:{goal:work.goal,tools:work.authorized_tools,spendingLimit:work.spending_limit},requested_by_agent_key:"supervisor"});}
...
async function finish(db:SupabaseClient,work:WorkItem,outcome:Record<string,unknown>,validation:Record<string,unknown>={status:"passed"}){
  const completed=now();
  await db.from("agent_work_steps").update({status:"succeeded",completed_at:completed,output:outcome,validation,updated_at:completed}).eq("work_item_id",work.id).in("status",["ready","running","waiting","pending"]);
...
async function executeWork(db:SupabaseClient,work:WorkItem){
  ...
  if(work.work_type==="technical.audit"){
    ...
    const findings={pages:pages.data.length,missingTitles:pages.data.filter(page=>!page.title).length,missingDescriptions:pages.data.filter(page=>!page.meta_description).length,missingH1:pages.data.filter(page=>!page.h1).length,notIndexable:pages.data.filter(page=>page.indexable===false).length,invalidSchema:pages.data.filter(page=>page.schema_json_ld_valid===false).length};
    await toolExecution(db,work,"seo.validate","succeeded",findings);await finish(db,work,{summary:`Technical audit completed across ${findings.pages} pages.`,findings},{status:"passed",evidenceCount:findings.pages});return;
  }
  ...
  if(work.work_type==="strategy.roadmap"){
    ...
    const items=opportunities.data,roadmap={days30:items.slice(0,5),days60:items.slice(5,12),days90:items.slice(12,20)};await toolExecution(db,work,"strategy.plan","succeeded",{counts:{days30:roadmap.days30.length,days60:roadmap.days60.length,days90:roadmap.days90.length}});await finish(db,work,{summary:"Evidence-backed 30, 60, and 90-day roadmap prepared.",roadmap},{status:"passed",prioritized:Math.min(items.length,20)});return;
  }
  if(work.work_type==="content.plan"){
    const [opportunities,client]=await Promise.all([db.from("seo_opportunities").select("id,action_type,opportunity_score,evidence").eq("project_id",work.project_id).in("action_type",["BUILD","CONTENT","IMPROVE","LOCALIZE"]).order("opportunity_score",{ascending:false}).limit(12),db.from("clients").select("organization_id").eq("id",work.client_id).eq("agency_id",work.agency_id).single()]);if(!opportunities.data?.length){await waitForEvidence(db,work,"The Content Agent is waiting for approved content opportunities.");return;}if(!client.data?.organization_id)throw new ApiError("The Content Agent could not resolve the client organization.",409,"CONFLICT");
    ...
    await toolExecution(db,work,"proof.read","succeeded",{opportunities:opportunities.data.length});await toolExecution(db,work,"creative.spec","succeeded",{specificationCount:specifications.length,ready:specifications.filter(item=>item.status==="ready").length});await finish(db,work,{summary:`Compiled ${specifications.length} intent-specific creative specifications. Copy generation remains evidence-gated and review-gated.`,specifications},{status:"passed",claimsPublished:0});return;
  }
  ...
  if(work.work_type==="implementation.change"){
    const packages=await db.from("implementation_packages").select("id,title:package_data,status,implementation_path").eq("project_id",work.project_id).in("status",["client_approved","approved"]).limit(10);if(!packages.data?.length){await waitForEvidence(db,work,"The Implementation Agent needs an approved implementation package.");return;}await toolExecution(db,work,"cms.draft","succeeded",{packages:packages.data.length});await finish(db,work,{summary:"Approved implementation package is ready for the connected execution pathway.",packages:packages.data},{status:"warning",publishing:"Requires the existing deployment or CMS execution flow"});return;
  }
  ...
  if(work.work_type==="reporting.summary"){
    ...
    const [events,opportunities,usage]=await Promise.all([db.from("proof_of_work_events").select("event_type,title,description,occurred_at").eq("project_id",work.project_id).order("occurred_at",{ascending:false}).limit(50),db.from("seo_opportunities").select("id,evidence,status").eq("project_id",work.project_id),db.from("data_usage_events").select("actual_cost,estimated_cost").eq("project_id",work.project_id)]);const spent=(usage.data??[]).reduce((sum,row)=>sum+num(row.actual_cost??row.estimated_cost),0),value=(opportunities.data??[]).reduce((sum,row)=>sum+num(asObject(row.evidence).estimated_monthly_value),0);await toolExecution(db,work,"report.generate","succeeded",{events:events.data?.length??0,spent,value});await finish(db,work,{summary:`Recorded ${events.data?.length??0} completed actions with $${spent.toFixed(2)} in provider spend and $${value.toFixed(0)} in directional monthly opportunity value.`,moneyUsed:spent,expectedMonthlyValue:value,recentWork:events.data??[]},{status:"passed",evidenceCount:events.data?.length??0});return;
  }
...
async function supervise(db:SupabaseClient,workItemId:string){
  ...
  if(!budget.allowed){
    const existing=await db.from("agent_approvals").select("id,status").eq("work_item_id",work.id).eq("approval_type","spending").order("requested_at",{ascending:false}).limit(1).maybeSingle();
    if(existing.data?.status==="rejected"){await db.from("agent_work_items").update({status:"blocked",final_outcome:{reason:"Spending approval was rejected."},updated_at:now()}).eq("id",work.id);return{workItemId,status:"blocked"};}
    if(existing.data?.status!=="approved"){
      if(!existing.data)await db.from("agent_approvals").insert({agency_id:work.agency_id,client_id:work.client_id,project_id:work.project_id,work_item_id:work.id,approval_type:"spending",title:"Approve agent spending",summary:`Requested limit $${num(work.spending_limit).toFixed(2)} exceeds the remaining monthly budget of $${budget.remaining.toFixed(2)}.`,risk_level:"high",requested_decision:budget,requested_by_agent_key:"supervisor"});
...
export async function processAgentBatch(size=10,workerId=`agents:${crypto.randomUUID()}`){
  ...
  for(const job of jobs){try{const workItemId=String(job.payload?.workItemId??"");if(!workItemId)throw new ApiError("Agent job is missing its work item.",500,"OPERATION_FAILED");const output=await supervise(db,workItemId);await db.from("background_jobs").update({status:"succeeded",completed_at:now(),worker_id:null,locked_at:null,lock_expires_at:null,updated_at:now()}).eq("id",job.id);results.push({jobId:job.id,...output});}catch(error){const safe=safeError(error),retryable=(safe.status===429||safe.status>=500)&&job.attempt_count<job.max_attempts,delay=Math.min(900_000,15_000*2**Math.max(0,job.attempt_count-1))+Math.floor(Math.random()*5000),status=retryable?"retry_scheduled":job.attempt_count>=job.max_attempts?"dead_letter":"failed";await db.from("background_jobs").update({status,available_at:new Date(Date.now()+delay).toISOString(),last_error_code:safe.body.error.code,last_error_message:safe.body.error.message,worker_id:null,locked_at:null,lock_expires_at:null,updated_at:now()}).eq("id",job.id);const workItemId=String(job.payload?.workItemId??"");if(workItemId&&!retryable)await db.from("agent_work_items").update({status:status==="dead_letter"?"dead_letter":"failed",failed_at:now(),final_outcome:{code:safe.body.error.code,message:safe.body.error.message},updated_at:now()}).eq("id",workItemId);results.push({jobId:job.id,status,error:safe.body.error});}}
