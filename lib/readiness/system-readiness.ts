...
export async function systemReadiness(projectId?: string) {
  ...
  if (projectFound && evidenceStatus.keywords === 0) warnings.push("No keywords are stored yet; campaign discovery will derive them from connected evidence.");
  if (projectFound && evidenceStatus.rankingSnapshots === 0) warnings.push("No ranking snapshots are stored yet; Search Console or authorized domain discovery is recommended.");
  const ready = blockers.length === 0;
  const recommendedNextStep=!ready?blockers[0]:evidenceStatus.keywords===0?(evidenceStatus.searchConsoleRows>0?"Run automatic discovery from Search Console evidence.":hasDataForSeoConfig?"Authorize automatic domain discovery.":"Connect Search Console or DataForSEO for zero-input discovery."):"Review the highest eligible opportunity.";
  return { ready, projectFound, databaseReady: hasSupabaseAdminConfig && missingTables.length === 0, providerStatus: { dataForSeo: hasDataForSeoConfig, searchConsole: evidenceStatus.searchConsoleRows > 0, scheduler: Boolean(env.CRON_SECRET) }, evidenceStatus, blockers, warnings, recommendedNextStep, configuration: { supabase: hasSupabaseAdminConfig, dataForSeo: hasDataForSeoConfig, github: hasGitHubConfig, scheduler: Boolean(env.CRON_SECRET), vercel: Boolean(env.VERCEL_PROJECT_ID && env.VERCEL_WEBHOOK_SECRET) }, missingTables, evidence: { keywords:evidenceStatus.keywords,metrics:evidenceStatus.keywordMetrics,rankings:evidenceStatus.rankingSnapshots,pages:evidenceStatus.pages,competitors:evidenceStatus.competitors,audits:evidenceStatus.audits } };
}
