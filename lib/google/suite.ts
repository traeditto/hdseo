...
import { nextProviderSyncAt } from "@/lib/outcomes/provider-health";
...
export async function loadSuiteConnection(db:SupabaseClient,input:{agencyId:string;clientId:string;projectId:string;provider:"google_analytics"|"google_business_profile"}){
  const result=await db.from("integration_connections").select("id,agency_id,client_organization_id,project_id,selected_resource,encrypted_secret_reference,status,last_synced_at,last_sync_attempt_at,next_sync_at,consecutive_sync_failures,last_sync_error_code,metadata").eq("agency_id",input.agencyId).eq("client_organization_id",input.clientId).eq("project_id",input.projectId).eq("provider",input.provider).eq("status","active").maybeSingle();
...
export async function discoverSuiteResources(db:SupabaseClient,tenant:{agencyId:string;clientId:string;projectId:string},provider:"google_analytics"|"google_business_profile"){
  const connection=await loadSuiteConnection(db,{...tenant,provider}),currentMetadata=(connection.metadata as Record<string,unknown>)??{},startedAt=new Date(),run=await db.from("provider_sync_runs").insert({agency_id:tenant.agencyId,client_organization_id:tenant.clientId,project_id:tenant.projectId,connection_id:connection.id,provider,operation:"resource_discovery"}).select("id").single();
  if(run.error||!run.data)throw new ApiError("Google resource discovery could not be started.",500,"DATABASE_BINDING_FAILED");
  try{
    const accessToken=await suiteAccess(db,connection);
    if(provider==="google_analytics"){
      const properties=await listAnalyticsProperties(accessToken),selected=connection.selected_resource??(properties.length===1?properties[0].property:null),metadata={...currentMetadata,properties,health:selected?"ready":properties.length?"property_selection_required":"property_access_required",lastDiscoveryAt:startedAt.toISOString()};
      const saved=await db.from("integration_connections").update({selected_resource:selected,metadata,last_verified_at:startedAt.toISOString(),last_sync_error_code:null,last_sync_error_message:null,updated_at:startedAt.toISOString()}).eq("id",connection.id);if(saved.error)throw new ApiError("GA4 discovery results could not be saved.",500,"DATABASE_BINDING_FAILED");
      await db.from("provider_sync_runs").update({status:"succeeded",records_read:properties.length,records_written:properties.length,metadata:{selected:Boolean(selected)},completed_at:new Date().toISOString()}).eq("id",run.data.id);
      return{selected,resources:properties.length};
    }
    const accounts=await listBusinessAccounts(accessToken),locations=(await Promise.all(accounts.slice(0,20).map(async account=>(await listBusinessLocations(accessToken,account.name!)).map(item=>({raw:item,account:account.name,accountName:account.accountName}))))).flat(),selected=connection.selected_resource??(locations.length===1?String(locations[0].raw.name??""):null),selectedAccount=locations.length===1?String(locations[0].account):accounts.length===1?accounts[0].name:null,metadata={...currentMetadata,accounts,selectedAccount,locations:locations.map(item=>({name:item.raw.name,title:item.raw.title,account:item.account,accountName:item.accountName})),health:selected?"ready":locations.length?"location_selection_required":"location_access_required",lastDiscoveryAt:startedAt.toISOString()};
    const saved=await db.from("integration_connections").update({selected_resource:selected,metadata,last_verified_at:startedAt.toISOString(),last_sync_error_code:null,last_sync_error_message:null,updated_at:startedAt.toISOString()}).eq("id",connection.id);if(saved.error)throw new ApiError("Business Profile discovery results could not be saved.",500,"DATABASE_BINDING_FAILED");
    await db.from("provider_sync_runs").update({status:"succeeded",records_read:locations.length,records_written:locations.length,metadata:{accounts:accounts.length,selected:Boolean(selected)},completed_at:new Date().toISOString()}).eq("id",run.data.id);
    return{selected,resources:locations.length};
  }catch(error){
    const completedAt=new Date(),errorCode=error instanceof ApiError?error.code:"GOOGLE_API_FAILED";
    await db.from("provider_sync_runs").update({status:"failed",error_code:errorCode,error_message:"Google resource discovery failed.",completed_at:completedAt.toISOString()}).eq("id",run.data.id);
    await db.from("integration_connections").update({last_sync_attempt_at:completedAt.toISOString(),next_sync_at:nextProviderSyncAt(completedAt,1),last_sync_error_code:errorCode,last_sync_error_message:"Google resource discovery failed.",metadata:{...currentMetadata,health:"discovery_failed",lastFailureAt:completedAt.toISOString()},updated_at:completedAt.toISOString()}).eq("id",connection.id);
    throw error;
  }
}

export async function syncAnalytics(db:SupabaseClient,tenant:{agencyId:string;clientId:string;projectId:string}){
  const connection=await loadSuiteConnection(db,{...tenant,provider:"google_analytics"}),property=connection.selected_resource;if(!property)throw new ApiError("Choose a GA4 property before syncing.",409,"PROPERTY_NOT_AUTHORIZED");
  ...
  if(run.error||!run.data)throw new ApiError("The GA4 synchronization could not be started.",500,"DATABASE_BINDING_FAILED");
  try{const accessToken=await suiteAccess(db,connection),report=await runAnalyticsReport(accessToken,property),rows=(report.rows??[]).map(row=>{const[d,landing,source,medium]=row.dimensionValues??[],[sessions,conversions,revenue]=row.metricValues??[];const raw=d?.value??"";return{agency_id:tenant.agencyId,client_organization_id:tenant.clientId,project_id:tenant.projectId,provider:"ga4",metric_date:raw.length===8?`${raw.slice(0,4)}-${raw.slice(4,6)}-${raw.slice(6,8)}`:new Date().toISOString().slice(0,10),landing_page:landing?.value??"",source:source?.value??"",medium:medium?.value??"",campaign:"",sessions:Number(sessions?.value)||0,organic_sessions:Number(sessions?.value)||0,conversions:Number(conversions?.value)||0,revenue:Number(revenue?.value)||0,metadata:{property},captured_at:new Date().toISOString()}});if(rows.length){const saved=await db.from("analytics_daily_metrics").upsert(rows,{onConflict:"project_id,provider,metric_date,landing_page,source,medium,campaign"});if(saved.error)throw saved.error;}const completedAt=new Date();await db.from("provider_sync_runs").update({status:"succeeded",records_read:rows.length,records_written:rows.length,completed_at:completedAt.toISOString()}).eq("id",run.data?.id);await db.from("integration_connections").update({last_synced_at:completedAt.toISOString(),last_verified_at:completedAt.toISOString(),last_sync_attempt_at:completedAt.toISOString(),next_sync_at:nextProviderSyncAt(completedAt),consecutive_sync_failures:0,last_sync_error_code:null,last_sync_error_message:null,updated_at:completedAt.toISOString()}).eq("id",connection.id);return{records:rows.length,property};}catch(error){await db.from("provider_sync_runs").update({status:"failed",error_code:"GOOGLE_API_FAILED",error_message:"Google Analytics synchronization failed.",completed_at:new Date().toISOString()}).eq("id",run.data?.id);throw error;}
}

export async function syncBusinessProfile(db:SupabaseClient,tenant:{agencyId:string;clientId:string;projectId:string}){
  const connection=await loadSuiteConnection(db,{...tenant,provider:"google_business_profile"}),account=String((connection.metadata as Record<string,unknown>)?.selectedAccount??"");if(!account)throw new ApiError("Choose a Business Profile account before syncing.",409,"PROPERTY_NOT_AUTHORIZED");
