...
type EvidenceJob={id:string;job_type:string;agency_id:string;client_organization_id:string|null;project_id:string|null;website_id:string|null;source_connection_id:string|null;payload:Record<string,unknown>;attempt_count:number;max_attempts:number};
...
export async function processEvidenceBatch(size=env.AUTOMATION_JOB_BATCH_SIZE,workerId=`evidence:${crypto.randomUUID()}`){
  ...
  for(const job of jobs){const handler=handlers[job.job_type];try{if(!handler)throw new ApiError(`Unknown evidence job type: ${job.job_type}`,500,"OPERATION_FAILED");const output=await handler(job);await db.from("background_jobs").update({status:"succeeded",completed_at:new Date().toISOString(),worker_id:null,locked_at:null,lock_expires_at:null,updated_at:new Date().toISOString()}).eq("id",job.id);if(job.project_id)await resumeWaitingCampaigns(db,job.project_id);results.push({jobId:job.id,status:"succeeded",output});logEvent("evidence_job_completed",{jobId:job.id,agencyId:job.agency_id,projectId:job.project_id??undefined,stage:job.job_type});}catch(error){const safe=safeError(error),retryable=(safe.status===429||safe.status>=500)&&job.attempt_count<job.max_attempts,delay=Math.min(900_000,15_000*2**Math.max(0,job.attempt_count-1))+Math.floor(Math.random()*5000),status=retryable?"retry_scheduled":job.attempt_count>=job.max_attempts?"dead_letter":"failed";await db.from("background_jobs").update({status,available_at:new Date(Date.now()+delay).toISOString(),last_error_code:safe.body.error.code,last_error_message:safe.body.error.message,worker_id:null,locked_at:null,lock_expires_at:null,updated_at:new Date().toISOString()}).eq("id",job.id);const referenceId=logServerError("evidence_job_failed",error,{jobId:job.id,agencyId:job.agency_id,projectId:job.project_id??undefined,operation:job.job_type,errorCode:safe.body.error.code});results.push({jobId:job.id,status,error:safe.body.error,referenceId});}}
