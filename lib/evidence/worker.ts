...
async function resumeWaitingCampaigns(db:SupabaseClient,job:EvidenceJob){
  if(!job.project_id||!job.client_organization_id)return;
  ...
  const resumed=await db.from("seo_campaign_jobs").update({status:"queued",next_attempt_at:timestamp,heartbeat_at:timestamp,updated_at:timestamp}).eq("project_id",projectId).eq("status","awaiting_evidence_refresh").select("id");
  if(resumed.error)throw new ApiError("The evidence-ready campaign could not be resumed.",503,"DATABASE_BINDING_FAILED");
  await wakeManagedAgentService(db,{agencyId:job.agency_id,clientId:job.client_organization_id,projectId,reason:"evidence_ready"});
  logEvent("evidence_ready_workflows_resumed",{agencyId:job.agency_id,projectId,status:"scheduled",resumedCampaigns:resumed.data?.length??0});
}
