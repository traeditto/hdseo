...
export async function dataForSeoRequest<T>(path: string, body: unknown, tag: string): Promise<NormalizedProviderResult<T>> {
  ...
  try {
    ...
    const response = await fetch(`${BASE_URL}${path}`, { method: "POST", headers: { Authorization: `Basic ${token}`, "Content-Type": "application/json", "X-Request-Tag": tag }, body: JSON.stringify(body), signal: controller.signal, cache: "no-store" });
