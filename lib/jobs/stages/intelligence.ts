...
import { keywordQualityIssues } from "@/lib/seo/keyword-quality";
import {
...
const opportunityDisqualifiers=(keyword:string,investment:{reasons:string[]})=>[
  ...keywordQualityIssues(keyword),
  ...investment.reasons,
];
...
export async function scoreStage(db:SupabaseClient,job:CampaignJob){
  ...
  const [keywordResult,metricResult,rankingResult,pageResult,competitorResult,auditResult,campaignResult,analyticsResult,leadResult,gscResult,profileResult,subscriptionResult]=await Promise.all([
    ...
    db.from("client_growth_profiles").select("average_customer_value").eq("agency_id",job.agency_id).eq("client_organization_id",job.client_organization_id).eq("project_id",job.project_id).maybeSingle(),
    db.from("client_subscriptions").select("plan_key").eq("agency_id",job.agency_id).eq("client_organization_id",job.client_organization_id).eq("project_id",job.project_id).maybeSingle(),
  ]);
  ...
  const analytics=(analyticsResult.data??[]).reduce((sum,row)=>({sessions:sum.sessions+Number(row.organic_sessions??0),conversions:sum.conversions+Number(row.conversions??0)}),{sessions:0,conversions:0}),leads=leadResult.data??[],qualified=leads.filter(row=>row.qualified).length,sales=leads.filter(row=>Number(row.gross_profit??0)>0||/won|closed|booked/i.test(String(row.status))).length,totalGrossProfit=leads.reduce((sum,row)=>sum+Number(row.gross_profit??0),0),campaignEconomics=record(campaignResult.data?.business_economics),profileGrossProfit=numeric(profileResult.data?.average_customer_value)==null?null:Number(profileResult.data?.average_customer_value)*.35,observed:EconomicAssumptions={leadConversionRate:analytics.sessions>0?analytics.conversions/analytics.sessions:null,qualifiedLeadRate:leads.length?qualified/leads.length:null,closeRate:qualified?sales/qualified:null,grossProfitPerSale:sales?totalGrossProfit/sales:profileGrossProfit,implementationCost:numeric(campaignResult.data?.implementation_budget)};
  ...
  const investmentPolicy=investmentPolicyForPlan(String(subscriptionResult.data?.plan_key??"agency_core"));
  ...
  for(const keyword of (keywordResult.data??[]) as KeywordRow[]){
    const metric=latestMetric.get(keyword.id),ranks=rankingHistory.get(keyword.id)??[],current=ranks[0],previous=ranks[1],ownership=analyzePageOwnership(keyword.keyword,pages);
    const result=scoreOpportunity({currentRank:current?.position==null?null:Number(current.position),previousRank:previous?.position==null?null:Number(previous.position),searchVolume:metric?.search_volume==null?null:Number(metric.search_volume),cpc:metric?.cpc==null?null:Number(metric.cpc),commercialIntentScore:keyword.commercial_intent_score,serviceRelevance:keyword.service_id?90:55,locationRelevance:keyword.location_id?90:55,competitorGap:(competitorResult.count??0)>0?70:null,technicalReadiness:auditResult.data?.[0]?.score==null?null:Number(auditResult.data[0].score),hasOwnerPage:Boolean(ownership.ownerPage),internalLinkCount:ownership.ownerPage?.internalLinks?.length??null});
    const effectiveTargetUrl=result.actionType==="BUILD"?keyword.target_url:ownership.ownerPage?.url??keyword.target_url,gsc=queryEvidence.get(normalizedQuery(keyword.keyword));
    const tacticalValue=valueOpportunity({seoScore:result.opportunityScore,confidenceScore:result.confidenceScore,searchVolume:numeric(metric?.search_volume),impressions:gsc?.impressions?gsc.impressions/3:null,currentCtr:gsc?.impressions?gsc.clicks/gsc.impressions:null,currentRank:numeric(current?.position),targetMilestone:result.targetMilestone,actionType:result.actionType,economics});
    const currentRank=numeric(current?.position),searchVolume=numeric(metric?.search_volume,0)??0,strategicFocus=Boolean(result.actionType==="IMPROVE"&&keyword.service_id&&keyword.location_id&&effectiveTargetUrl&&currentRank!=null&&currentRank>investmentPolicy.maximumExistingPageRank&&currentRank<=investmentPolicy.maximumStrategicFocusRank&&searchVolume>=500);
    const strategicValue=strategicFocus?valueOpportunity({seoScore:result.opportunityScore,confidenceScore:result.confidenceScore,searchVolume,impressions:gsc?.impressions?gsc.impressions/3:null,currentCtr:gsc?.impressions?gsc.clicks/gsc.impressions:null,currentRank,targetMilestone:"Top 10",actionType:result.actionType,economics:{...economics,implementationCost:Math.max(900,tacticalValue.businessValue.implementationCost*3)}}):null;
    const value=strategicValue??tacticalValue;
    const investment=evaluateSeoInvestment({expectedMonthlyProfit:value.businessValue.expectedMonthlyProfit,implementationCost:value.businessValue.implementationCost,paybackMonths:value.businessValue.paybackMonths,confidenceScore:result.confidenceScore,currentRank,actionType:result.actionType,economicsConfidence:value.economicsConfidence,opportunityScore:value.combinedScore,strategicFocus},investmentPolicy);
    const key=opportunityKey(job.project_id,keyword.keyword,effectiveTargetUrl,result.actionType),existing=await db.from("seo_opportunities").select("id,status,cooldown_until,evidence").eq("project_id",job.project_id).eq("opportunity_key",key).in("status",["open","selected","approved","in_progress","monitoring"]).maybeSingle();
    const disqualifiers=[...opportunityDisqualifiers(keyword.keyword,investment),...(result.actionType==="BUILD"&&!keyword.service_id?["SERVICE_NOT_VERIFIED"]:[])],eligibility=evaluateEligibility({projectId:job.project_id,keyword:keyword.keyword,targetUrl:effectiveTargetUrl,result,pageConflict:ownership.conflictWarnings.length>0,activeDuplicate:false,cooldownUntil:existing.data?.cooldown_until,evidenceRequired:result.missingEvidence.length>3?result.missingEvidence:[],minimumConfidence:investmentPolicy.minimumConfidence,disqualifiers});
    const scoreBreakdown={...Object.fromEntries(result.evidence.map((factor)=>[factor.label,factor.points])),"Expected profit":value.businessValue.priorityScore,"Investment focus":investment.focusScore},priority=investment.focusScore>=85?"critical":investment.focusScore>=70?"high":investment.focusScore>=55?"medium":"low",focusCampaign=strategicFocus?{active:true,objective:`Move “${keyword.keyword}” through positions 40, 20 and into the top 10.`,milestones:["Top 40","Top 20","Top 10"],targetMilestone:"Top 10",estimatedPhases:3,tacticalBusinessValue:tacticalValue.businessValue,modeledCampaignCost:value.businessValue.implementationCost}:null,priorEvidence=record(existing.data?.evidence),evidence={keyword:keyword.keyword,currentRank:current?.position??null,rankingUrl:current?.ranking_url??null,searchVolume:metric?.search_volume??null,impressions:gsc?.impressions??null,ctr:gsc?.impressions?gsc.clicks/gsc.impressions:null,ownership,missingEvidence:result.missingEvidence,scoreBreakdown,businessValue:value.businessValue,economicsConfidence:value.economicsConfidence,valueExplanation:strategicFocus?`HD SEO modeled a concentrated, three-phase campaign around this verified local service keyword instead of valuing only the next small ranking step. The campaign must reach staged ranking milestones and remains subject to measurement and rollback rules.`:value.explanation,economicAssumptions:value.assumptions,investmentDecision:investment,focusCampaign,customerFocus:priorEvidence.customerFocus??null};
    const values={agency_id:job.agency_id,client_organization_id:job.client_organization_id,project_id:job.project_id,keyword_id:keyword.id,opportunity_score:investment.focusScore,confidence_score:result.confidenceScore,action_type:result.actionType,priority,target_milestone:strategicFocus?"Top 10":result.targetMilestone,reason_codes:[...new Set([...result.reasonCodes,...disqualifiers])],evidence,recommended_actions:strategicFocus?[`Concentrate page, content, internal-link and technical improvements on “${keyword.keyword}”.`,`Advance through measured Top 40, Top 20 and Top 10 milestones before switching objectives.`,...result.recommendedActions]:result.recommendedActions,status:existing.data?.status??"open",scoring_version:"4.1-strategic-focus",opportunity_key:key,target_url:effectiveTargetUrl};
    ...
    if(saved.data)candidates.push({job_id:job.id,opportunity_id:saved.data.id,eligibility_status:eligibility.eligible?"eligible":"blocked",score:investment.focusScore,confidence:result.confidenceScore,target_milestone:result.targetMilestone,score_breakdown:scoreBreakdown,evidence,missing_evidence:result.missingEvidence,deferred_reason:eligibility.reasons.join(", ")||null,targetUrl:effectiveTargetUrl});
  }
...
export async function prepareStage(db:SupabaseClient,job:CampaignJob){
  ...
  if(managedOutcome){
    ...
    return pause(db,{...job,result:nextResult},"awaiting_manual_completion",{...nextResult,packageId:created.id,taskId:created.taskId,approvalOwner,approvalQuestion:approvalOwner==="client"?"Approve the prepared change package, or ask HD SEO to revise it.":"The accountable agency reviewer must approve this package before client publication."});
  }
