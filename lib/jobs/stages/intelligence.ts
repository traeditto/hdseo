...
const opportunityDisqualifiers=(keyword:string,businessValue:{paybackMonths:number|null;expectedMonthlyProfit:number})=>[
  ...keywordQualityIssues(keyword),
  ...(businessValue.expectedMonthlyProfit<=0?["NO_EXPECTED_BUSINESS_VALUE"]:[]),
  ...(businessValue.paybackMonths!=null&&businessValue.paybackMonths>60?["PAYBACK_EXCEEDS_AUTOPILOT_LIMIT"]:[]),
];
...
export async function scoreStage(db:SupabaseClient,job:CampaignJob){
  ...
  for(const keyword of (keywordResult.data??[]) as KeywordRow[]){
    const metric=latestMetric.get(keyword.id),ranks=rankingHistory.get(keyword.id)??[],current=ranks[0],previous=ranks[1],ownership=analyzePageOwnership(keyword.keyword,pages),result=scoreOpportunity({currentRank:current?.position==null?null:Number(current.position),previousRank:previous?.position==null?null:Number(previous.position),searchVolume:metric?.search_volume==null?null:Number(metric.search_volume),cpc:metric?.cpc==null?null:Number(metric.cpc),commercialIntentScore:keyword.commercial_intent_score,serviceRelevance:keyword.service_id?90:55,locationRelevance:keyword.location_id?90:55,competitorGap:(competitorResult.count??0)>0?70:null,technicalReadiness:auditResult.data?.[0]?.score==null?null:Number(auditResult.data[0].score),hasOwnerPage:Boolean(ownership.ownerPage),internalLinkCount:ownership.ownerPage?.internalLinks?.length??null}),effectiveTargetUrl=result.actionType==="BUILD"?keyword.target_url:ownership.ownerPage?.url??keyword.target_url,gsc=queryEvidence.get(normalizedQuery(keyword.keyword)),value=valueOpportunity({seoScore:result.opportunityScore,confidenceScore:result.confidenceScore,searchVolume:numeric(metric?.search_volume),impressions:gsc?.impressions?gsc.impressions/3:null,currentCtr:gsc?.impressions?gsc.clicks/gsc.impressions:null,currentRank:numeric(current?.position),targetMilestone:result.targetMilestone,actionType:result.actionType,economics}),key=opportunityKey(job.project_id,keyword.keyword,effectiveTargetUrl,result.actionType),existing=await db.from("seo_opportunities").select("id,status,cooldown_until").eq("project_id",job.project_id).eq("opportunity_key",key).in("status",["open","approved","in_progress","monitoring"]).maybeSingle(),disqualifiers=[...opportunityDisqualifiers(keyword.keyword,value.businessValue),...(result.actionType==="BUILD"&&!keyword.service_id?["SERVICE_NOT_VERIFIED"]:[])],eligibility=evaluateEligibility({projectId:job.project_id,keyword:keyword.keyword,targetUrl:effectiveTargetUrl,result,pageConflict:ownership.conflictWarnings.length>0,activeDuplicate:false,cooldownUntil:existing.data?.cooldown_until,evidenceRequired:result.missingEvidence.length>3?result.missingEvidence:[],disqualifiers}),scoreBreakdown={...Object.fromEntries(result.evidence.map((factor)=>[factor.label,factor.points])),"Expected profit":value.businessValue.priorityScore},priority=value.combinedScore>=90?"critical":value.combinedScore>=75?"high":value.combinedScore>=55?"medium":"low",evidence={keyword:keyword.keyword,currentRank:current?.position??null,rankingUrl:current?.ranking_url??null,searchVolume:metric?.search_volume??null,impressions:gsc?.impressions??null,ctr:gsc?.impressions?gsc.clicks/gsc.impressions:null,ownership,missingEvidence:result.missingEvidence,scoreBreakdown,businessValue:value.businessValue,economicsConfidence:value.economicsConfidence,valueExplanation:value.explanation,economicAssumptions:value.assumptions};
    const values={agency_id:job.agency_id,client_organization_id:job.client_organization_id,project_id:job.project_id,keyword_id:keyword.id,opportunity_score:value.combinedScore,confidence_score:result.confidenceScore,action_type:result.actionType,priority,target_milestone:result.targetMilestone,reason_codes:[...result.reasonCodes,...disqualifiers],evidence,recommended_actions:result.recommendedActions,status:existing.data?.status??"open",scoring_version:"3.2-target",opportunity_key:key,target_url:effectiveTargetUrl};
