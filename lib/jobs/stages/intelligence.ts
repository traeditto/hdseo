...
export async function scoreStage(db:SupabaseClient,job:CampaignJob){
  ...
  for(const keyword of (keywordResult.data??[]) as KeywordRow[]){
    ...
    const value=valueOpportunity({seoScore:result.opportunityScore,confidenceScore:result.confidenceScore,searchVolume:numeric(metric?.search_volume),impressions:gsc?.impressions?gsc.impressions/3:null,currentCtr:gsc?.impressions?gsc.clicks/gsc.impressions:null,currentRank:numeric(current?.position),targetMilestone:result.targetMilestone,actionType:result.actionType,economics});
    const investment=evaluateSeoInvestment({expectedMonthlyProfit:value.businessValue.expectedMonthlyProfit,implementationCost:value.businessValue.implementationCost,paybackMonths:value.businessValue.paybackMonths,confidenceScore:result.confidenceScore,currentRank:numeric(current?.position),actionType:result.actionType,economicsConfidence:value.economicsConfidence,opportunityScore:value.combinedScore},investmentPolicy);
    ...
    const scoreBreakdown={...Object.fromEntries(result.evidence.map((factor)=>[factor.label,factor.points])),"Expected profit":value.businessValue.priorityScore,"Investment focus":investment.focusScore},priority=investment.focusScore>=85?"critical":investment.focusScore>=70?"high":investment.focusScore>=55?"medium":"low",evidence={keyword:keyword.keyword,currentRank:current?.position??null,rankingUrl:current?.ranking_url??null,searchVolume:metric?.search_volume??null,impressions:gsc?.impressions??null,ctr:gsc?.impressions?gsc.clicks/gsc.impressions:null,ownership,missingEvidence:result.missingEvidence,scoreBreakdown,businessValue:value.businessValue,economicsConfidence:value.economicsConfidence,valueExplanation:value.explanation,economicAssumptions:value.assumptions,investmentDecision:investment};
    const values={agency_id:job.agency_id,client_organization_id:job.client_organization_id,project_id:job.project_id,keyword_id:keyword.id,opportunity_score:value.combinedScore,confidence_score:result.confidenceScore,action_type:result.actionType,priority,target_milestone:result.targetMilestone,reason_codes:[...new Set([...result.reasonCodes,...disqualifiers])],evidence,recommended_actions:result.recommendedActions,status:existing.data?.status??"open",scoring_version:"4.0-investment-focus",opportunity_key:key,target_url:effectiveTargetUrl};
