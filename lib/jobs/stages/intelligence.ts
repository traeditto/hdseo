...
export async function scoreStage(db:SupabaseClient,job:CampaignJob){
  ...
  for(const keyword of (keywordResult.data??[]) as KeywordRow[]){
    ...
    const key=opportunityKey(job.project_id,keyword.keyword,effectiveTargetUrl,result.actionType),existing=await db.from("seo_opportunities").select("id,status,cooldown_until").eq("project_id",job.project_id).eq("opportunity_key",key).in("status",["open","approved","in_progress","monitoring"]).maybeSingle();
    ...
    const scoreBreakdown={...Object.fromEntries(result.evidence.map((factor)=>[factor.label,factor.points])),"Expected profit":value.businessValue.priorityScore,"Investment focus":investment.focusScore},priority=investment.focusScore>=85?"critical":investment.focusScore>=70?"high":investment.focusScore>=55?"medium":"low",focusCampaign=strategicFocus?{active:true,objective:`Move “${keyword.keyword}” through positions 40, 20 and into the top 10.`,milestones:["Top 40","Top 20","Top 10"],targetMilestone:"Top 10",estimatedPhases:3,tacticalBusinessValue:tacticalValue.businessValue,modeledCampaignCost:value.businessValue.implementationCost}:null,evidence={keyword:keyword.keyword,currentRank:current?.position??null,rankingUrl:current?.ranking_url??null,searchVolume:metric?.search_volume??null,impressions:gsc?.impressions??null,ctr:gsc?.impressions?gsc.clicks/gsc.impressions:null,ownership,missingEvidence:result.missingEvidence,scoreBreakdown,businessValue:value.businessValue,economicsConfidence:value.economicsConfidence,valueExplanation:strategicFocus?`HD SEO modeled a concentrated, three-phase campaign around this verified local service keyword instead of valuing only the next small ranking step. The campaign must reach staged ranking milestones and remains subject to measurement and rollback rules.`:value.explanation,economicAssumptions:value.assumptions,investmentDecision:investment,focusCampaign};
