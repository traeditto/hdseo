...
export async function processOneCampaignJob(workerId=crypto.randomUUID()){
  ...
  catch(error){const safe=safeError(error),nextAttempt=job.attempt_count+1,retryable=safe.status>=500&&nextAttempt<job.max_attempts;await db.from("seo_campaign_jobs").update({status:retryable?"retry_scheduled":"failed",attempt_count:nextAttempt,error_code:safe.body.error.code,error_message:safe.body.error.message,error_details:{referenceId:safe.body.error.referenceId},next_attempt_at:new Date(Date.now()+Math.min(300_000,30_000*Math.max(1,nextAttempt))).toISOString(),failed_at:retryable?null:new Date().toISOString(),worker_id:null,locked_at:null,lock_expires_at:null,heartbeat_at:new Date().toISOString()}).eq("id",job.id);logEvent("job_stage_failed",{jobId:job.id,stage:job.current_stage,status:retryable?"retry_scheduled":"failed",errorCode:safe.body.error.code,referenceId:job.reference_id});return{jobId:job.id,status:retryable?"retry_scheduled":"failed",error:safe.body.error};}
}
