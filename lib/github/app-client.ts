...
export async function githubRequest<T>(path:string,token:string,options:RequestInit={}) { const controller=new AbortController(),timer=setTimeout(()=>controller.abort(),15_000); try { const url=new URL(path,API);if(url.origin!==API)throw new ApiError("GitHub destination is not permitted.",400,"PROVIDER_HOST_FORBIDDEN");const response=await fetch(url,{...options,headers:{Accept:"application/vnd.github+json",Authorization:`Bearer ${token}`,"X-GitHub-Api-Version":"2022-11-28",...(options.headers??{})},signal:controller.signal,cache:"no-store",redirect:"error"}); if(!response.ok)throw new ApiError(`GitHub request failed with HTTP ${response.status}.`,response.status===429?429:502,response.status===429?"RATE_LIMITED":"OPERATION_FAILED",response.headers.get("x-github-request-id")??undefined);if(Number(response.headers.get("content-length")||0)>10_000_000)throw new ApiError("GitHub response exceeded the permitted size.",502,"PROVIDER_RESPONSE_TOO_LARGE"); if(response.status===204||response.headers.get("content-length")==="0")return undefined as T;return await response.json() as T; } finally { clearTimeout(timer); } }
