...
export async function findExistingInstallation(context:GitHubManagementContext){
  ...
  if(stored.error)databaseBindingError("Existing GitHub installation status could not be loaded.",stored.error,context,"installation.existing_lookup");
  ...
  if(bound.error)databaseBindingError("GitHub installation ownership records could not be loaded.",bound.error,context,"installation.ownership_lookup");
