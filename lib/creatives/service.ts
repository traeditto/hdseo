...
export async function generateCreativeDraft(db:SupabaseClient,tenant:CreativeTenant,specId:string){
  ...
  try{
    const generated=await generateEvidenceConstrainedCreative({spec:{targetKeyword:spec.target_keyword,searchIntent:spec.search_intent,creativeAngle:spec.creative_angle,userJob:spec.user_job,ownerPageUrl:spec.owner_page_url,requiredSections:spec.required_sections,internalLinkPlan:spec.internal_link_plan,conversionGoal:spec.conversion_goal,schemaPlan:spec.schema_plan,restrictions:spec.restrictions},verifiedProof:usableProof,approvedClaims:claims.data??[]});
