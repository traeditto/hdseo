...
export async function releaseAutopilotPreview(db:SupabaseClient,input:{executionId:string;agencyId:string;clientId:string;projectId:string}){
  const execution=await db.from("seo_executions").select("id,status,repository_connection_id,pull_request_number,preview_deployment_id,validation_results,outcome_run_id,repository_mutation_intent_id,repository_action_digest,approved_at").eq("id",input.executionId).eq("agency_id",input.agencyId).eq("client_organization_id",input.clientId).eq("project_id",input.projectId).maybeSingle();
  ...
  const validation=object(execution.data.validation_results),repositoryApproval=object(validation.repositoryApproval),repositoryWrite=object(validation.repositoryWrite);
  const [connection,deployment,checks,approvalIntent]=await Promise.all([
    ...
    execution.data.repository_mutation_intent_id?db.from("mutation_intents").select("id,status,approved_by,action_digest").eq("id",execution.data.repository_mutation_intent_id).eq("agency_id",input.agencyId).eq("project_id",input.projectId).maybeSingle():Promise.resolve({data:null,error:null}),
  ]);
  const approvedBy=typeof repositoryApproval.approvedBy==="string"?repositoryApproval.approvedBy:approvalIntent.data?.approved_by??null,approvalDigest=typeof repositoryApproval.actionDigest==="string"?repositoryApproval.actionDigest:execution.data.repository_action_digest??approvalIntent.data?.action_digest??null;
  if(!execution.data.approved_at||!approvedBy||!approvalDigest||approvalIntent.data?.status==="rejected")throw new ApiError("The exact website change has no accountable customer approval.",409,"APPROVAL_REQUIRED");
  if(approvalIntent.data&&approvalIntent.data.action_digest!==approvalDigest)throw new ApiError("The immutable approval no longer matches the approved website change.",409,"INVALID_STATE");
